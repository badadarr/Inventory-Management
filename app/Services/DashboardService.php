...
class DashboardService
{
    ...
    {
        $currentYearProfit = Order::selectRaw('EXTRACT(MONTH FROM created_at) as month, SUM(profit) as total_profit')
            ->whereYear('created_at', Carbon::now()->year)
            ->where('created_at', '>=', Carbon::now()->subMonths(7))
            ->groupBy('month')
            ->pluck('total_profit', 'month');

        $lastYearProfit = Order::selectRaw('EXTRACT(MONTH FROM created_at) as month, SUM(profit) as total_profit')
