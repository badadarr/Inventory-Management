...
const form = useForm({
    ...
    paid_through: props.order.transaction?.paid_through || 'cash',
...
const addOrderItem = () => {
    ...
    if (existingIndex >= 0) {
        ...
        form.order_items[existingIndex].subtotal = 
            form.order_items[existingIndex].quantity * form.order_items[existingIndex].price;
    } else {
        // Add new item
        const price = itemPrice.value || selectedProduct.value.selling_price;
        form.order_items.push({
            ...
            subtotal: price * itemQuantity.value,
...
const updateItemQuantity = (index, newQuantity) => {
    ...
    form.order_items[index].quantity = newQuantity;
    form.order_items[index].subtotal = form.order_items[index].price * newQuantity;
};

const updateItemPrice = (index, newPrice) => {
    if (newPrice < 0) newPrice = 0;
    form.order_items[index].price = newPrice;
    form.order_items[index].subtotal = newPrice * form.order_items[index].quantity;
};
    ...
    <AuthenticatedLayout>
        ...
        <div class="flex flex-wrap">
            <div class="w-full px-4">
                <div class="relative -mt-16 flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-white">
                            ...
                            <div class="mb-8 p-6 border-2 border-orange-200 rounded-lg bg-orange-50">
                                ...
                                <div class="mb-4 p-4 bg-white rounded-lg border border-orange-300">
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                                        ...
                                        <div class="flex flex-col">
                                            <label class="text-sm font-medium text-gray-700 mb-1">Price ({{ getCurrency() }})</label>
                                            <input 
                                ...
                                <div class="overflow-x-auto">
                                    <table class="min-w-full bg-white border border-orange-300 rounded-lg">
                                        <thead class="bg-orange-100">
                                            <tr>
                                                <th class="px-4 py-3 text-left text-xs font-semibold text-orange-800 uppercase">#</th>
                                                <th class="px-4 py-3 text-left text-xs font-semibold text-orange-800 uppercase">Product</th>
                                                <th class="px-4 py-3 text-left text-xs font-semibold text-orange-800 uppercase">Code</th>
                                                <th class="px-4 py-3 text-center text-xs font-semibold text-orange-800 uppercase">Price</th>
                                                <th class="px-4 py-3 text-center text-xs font-semibold text-orange-800 uppercase">Quantity</th>
                                                <th class="px-4 py-3 text-right text-xs font-semibold text-orange-800 uppercase">Subtotal</th>
                                                <th class="px-4 py-3 text-center text-xs font-semibold text-orange-800 uppercase">Action</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-orange-200">
                                            ...
                                            <tr v-for="(item, index) in form.order_items" :key="index" class="hover:bg-orange-50">
                                                ...
                                                <td class="px-4 py-3 text-center">
                                                    ...
                                                    />
                                                    <span class="ml-1 text-xs text-gray-500">{{ item.unit_symbol }}</span>
                                                </td>
                                    ...
                                    <div class="bg-white p-6 rounded-lg border-2 border-pink-300 shadow-sm">
                                        ...
                                        <div class="space-y-3">
                                            <div class="flex justify-between items-center text-sm">
                                                ...
                                                <span class="font-semibold text-gray-900">{{ getCurrency() }} {{ numberFormat(subTotal) }}</span>
                                            </div>
                                            <div class="flex justify-between items-center text-sm">
                                                <span class="text-gray-600">Tax:</span>
                                                <span class="font-semibold text-gray-900">{{ getCurrency() }} {{ numberFormat(taxAmount) }}</span>
                                            </div>
                                            <div class="border-t border-gray-300 pt-3 flex justify-between items-center">
                                                <span class="text-lg font-bold text-gray-900">Grand Total:</span>
                                                <span class="text-xl font-bold text-pink-600">{{ getCurrency() }} {{ numberFormat(grandTotal) }}</span>
                                            </div>
                                            <div class="flex justify-between items-center text-sm pt-2 border-t border-gray-200">
                                                <span class="text-gray-600">Paid:</span>
                                                <span class="font-semibold text-emerald-600">{{ getCurrency() }} {{ numberFormat(form.paid) }}</span>
                                            </div>
                                            <div class="flex justify-between items-center text-sm">
