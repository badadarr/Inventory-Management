    ...
    <AuthenticatedLayout>
        ...
        <div class="flex flex-wrap">
            <div class="w-full px-4">
                <CardTable indexRoute="orders.index" :paginatedData="orders" :filters="filters"
                    :tableHeads="tableHeads">
                    <template #cardHeader>
                        <div class="flex justify-between items-center">
                            <h4 class="text-2xl">Apply filters({{ orders.total }})</h4>
                            <div class="flex gap-2">
                                <Button :href="route('orders.create')" buttonType="link"
                                    class="bg-emerald-500 hover:bg-emerald-600">
                                    <i class="fa fa-plus mr-2"></i>Create Order
                                </Button>
                                <Button :href="route('carts.index')" buttonType="link"
                    ...
                    <tr v-for="(order, index) in orders.data" :key="order.id">
                        ...
                        <TableData>
                            <span :class="order.due > 0 ? 'text-red-500 text-xl font-bold' : ''">{{ getCurrency() }}{{
                                numberFormat(order.due) }}</span>
                            <br>
                            <div class="flex gap-1" v-if="order.due > 0">
                                <Button @click="payDueOrderModal(order)" title="Pay Due" class="px-2">
                                    <i class="fa fa-money-bill-wave"></i>
                                </Button>
                                <Button @click="settleOrderModal(order)" type="red" class="px-2" title="Settle">
                        ...
                        <TableData>
                            ...
                            <span v-else-if="order.status === 'pending'"
                                class="text-xs font-semibold inline-block py-1 px-2 rounded text-amber-600 bg-amber-200 uppercase">
                            ...
                            <span v-else
                                class="text-xs font-semibold inline-block py-1 px-2 rounded text-gray-600 bg-gray-200 uppercase">{{
                                order.status }}</span>
                        </TableData>
                        <TableData>{{ formatDatetime(order.created_at) }}</TableData>
                        <TableData>
                            <div class="flex gap-2">
                                <Button @click="viewOrderItemsModal(order)" title="View Order Items"
                                    class="bg-blue-500 hover:bg-blue-600">
                                    <i class="fa fa-list"></i>
                                </Button>
                                <Button :href="route('orders.show', order.id)" buttonType="link" title="View Details"
                                    class="bg-indigo-500 hover:bg-indigo-600">
                                    <i class="fa fa-eye"></i>
                                </Button>
                                <Button :href="route('orders.edit', order.id)" buttonType="link" title="Edit Order"
                                    class="bg-emerald-500 hover:bg-emerald-600">
        ...
        <Modal
            ...
            :show="showOrderItemsModal" @close="closeModal" maxWidth="4xl" :showSubmitButton="false">
                ...
                <div class="block w-full overflow-x-auto">
                    ...
                    <table class="items-center w-full bg-transparent border-collapse">
                        ...
                        <tbody>
                            ...
                            <template
                                v-for="(orderItem, index) in selectedOrder?.orderItems || selectedOrder?.order_items || []"
                                :key="orderItem?.id || index">
                                <tr v-if="orderItem">
                                    <TableData class="text-left flex items-center"
                                        :title="orderItem.product?.name || 'N/A'">
                                        <img :src="orderItem.product?.photo || 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%22 height=%22100%22%3E%3Crect fill=%22%23ddd%22 width=%22100%22 height=%22100%22/%3E%3Ctext fill=%22%23999%22 x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22 font-size=%2218%22%3ENo Image%3C/svg%3E'"
                                            class="h-12 w-12 bg-white rounded-full border object-cover"
                                            alt="Product image" @error="$event.target.style.display = 'none'" />
                                        <span class="ml-3 font-bold text-blueGray-600">{{ orderItem.product?.name ?
                                            truncateString(orderItem.product.name, 15) : 'N/A' }}</span>
                                    </TableData>
                                    <TableData>{{ orderItem.product?.product_number || '-' }}</TableData>
                                    <TableData>{{ orderItem.product?.product_code || '-' }}</TableData>
                                    <TableData>
                                        Unit Price: <strong>{{ getCurrency() }}{{ numberFormat(orderItem.unit_price ||
                                            0) }}</strong>
                                        <br>
                                        Subtotal: <strong>{{ getCurrency() }}{{ numberFormat(orderItem.subtotal || 0)
                                            }}</strong>
                                    </TableData>
                                    <TableData>
                                        <strong>{{ numberFormat(orderItem.quantity) }}{{
                                            orderItem.product?.unit_type?.symbol || '' }}</strong>
                                    </TableData>
                                    <TableData>
                                        <Button v-if="orderItem.product_id"
                                            :href="route('products.edit', orderItem.product_id)" buttonType="link"
                                            preserveScroll title="View Product">
                                            <i class="fa fa-eye"></i>
                                        </Button>
        ...
        <!--Pay due-->
        <Modal title="Pay Due" :show="showPaymentModal" :formProcessing="form.processing" @close="closeModal"
            @submitAction="payOrderDue" maxWidth="sm">
            <div>
                <div class="flex mt-1">
                    <select id="paid_through" v-model="form.paid_through"
                        ...
                        <option v-for="(orderPaidByType, index) in orderPaidByTypes" :key="index"
                            :value="orderPaidByType.value">
                            {{ orderPaidByType.label }}
                        </option>
                    </select>
                    <input id="paid" placeholder="Enter paid amount" v-model="form.amount" @keyup.enter="payOrderDue"
                        type="text"
        ...
        <!--Settle Order-->
        <Modal title="Due Settlement" :show="showSettleModal" :formProcessing="form.processing" @close="closeModal"
            @submitAction="settleDuePayment" maxWidth="md" submitButtonText="Yes, settle it!">
