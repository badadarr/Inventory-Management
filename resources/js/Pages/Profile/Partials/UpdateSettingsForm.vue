...
<template>
    ...
    <form @submit.prevent="$inertia.put(route('settings.update'), form, { preserveScroll: true })">
        <div class="flex flex-wrap">
            ...
            <div v-for="(settingField, index) in settings.fields" :key="index" class="w-full lg:w-6/12 px-4">
                <div class="relative w-full mb-3">
                    <label class="block uppercase text-blueGray-600 text-xs font-bold mb-2" :for="settingField.value">
                        {{ settingField.label }}
                    </label>
                    <input :id="settingField.value" type="text"
